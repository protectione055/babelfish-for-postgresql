...
DropDatabaseLink(DropDatabaseLinkStmt *stmt)
{
	...
	/*
	 * Make the catalog changes visible before dropping the foreign server.
	 * Otherwise, dependency checks during server deletion might still see
	 * pg_depend entries pointing at this database link.
	 */
	CommandCounterIncrement();
